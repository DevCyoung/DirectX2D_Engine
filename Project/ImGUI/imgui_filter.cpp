...
namespace ImGui
{
	...
	void SelectComboAutoSelectDemo(const char* title, const std::vector<std::string>& vector, int& selected_item, bool* p_open)
	{
		...
		selected_item = -1;
		if (ImGui::ComboAutoSelect(title,
			selected_item, vector, item_getter1, autoselect_search_vector, ImGuiComboFlags_HeightLarge)) {
			/* Selection made */
		}
